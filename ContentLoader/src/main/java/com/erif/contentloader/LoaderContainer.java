...
public class LoaderContainer extends FrameLayout {
    ...
    private LoaderDrawable drawable;

    private static final int TYPE_BLINK = 0;
    private static final int TYPE_SHIMMER = 1;

    public LoaderContainer(@NonNull Context context) {
        super(context);
        init(context, null, 0, 0);
    }

    public LoaderContainer(@NonNull Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs, 0, 0);
    }

    public LoaderContainer(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init(context, null, defStyleAttr, 0);
    }

    public LoaderContainer(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        drawable = new LoaderDrawable();
        drawable.setCallback(this);
        if (attrs != null) {
            TypedArray typedArray = context.getTheme().obtainStyledAttributes(
                    attrs, R.styleable.LoaderContainer, defStyleAttr, defStyleRes
            );
            boolean autoStart;
            try {
                autoStart = typedArray.getBoolean(R.styleable.LoaderContainer_android_autoStart, false);
                loaderType = typedArray.getInt(R.styleable.LoaderContainer_loaderType, TYPE_BLINK);
                duration = typedArray.getInt(R.styleable.LoaderContainer_android_duration, loaderType == TYPE_SHIMMER ? 1200 : 600);
